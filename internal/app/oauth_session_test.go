...
func TestOAuthSessionBindsTokensToTheSelectedOfficialIssuer(t *testing.T) {
	t.Setenv("GETBEE_DISABLE_KEYRING", "1")
	store := &state.Store{Dir: t.TempDir()}
	r := &runner{ctx: context.Background(), out: io.Discard, errOut: io.Discard, store: store}
	client := beeapi.New(beeapi.OAuthIssuerAI)
	account, err := r.saveOAuthSession(beeapi.OAuthMetadata{Issuer: beeapi.OAuthIssuerAI}, beeapi.OAuthToken{
		AccessToken: "boa_ai", RefreshToken: "bor_ai", TokenType: "DPoP", ExpiresIn: 600,
		Scope: "account:profile:read offline_access",
	}, client)
	if err != nil {
		t.Fatal(err)
	}
	loaded, secret, err := r.loadOAuthSession()
	if err != nil || loaded.Issuer != beeapi.OAuthIssuerAI || secret.Issuer != beeapi.OAuthIssuerAI {
		t.Fatalf("AI issuer session was not preserved: account=%#v secret=%#v err=%v", loaded, secret, err)
	}

	account.Issuer = beeapi.OAuthIssuerDev
	if err := store.SaveOAuthAccount(account); err != nil {
		t.Fatal(err)
	}
	if _, _, err := r.loadOAuthSession(); err == nil || !strings.Contains(err.Error(), "issuer 不匹配") {
		t.Fatalf("an AI token was accepted after metadata switched to DEV: %v", err)
	}
}
...
func TestDisconnectCleansIncompleteOAuthMetadataWithoutTouchingAPIKeys(t *testing.T) {
	...
	if err := store.SaveOAuthAccount(state.OAuthAccount{
		Protocol: beeapi.OAuthAccountProtocol, Issuer: beeapi.OAuthIssuerDev, ClientID: beeapi.OAuthClientID,
...
func TestOAuthSessionRejectsTamperedIssuerBeforeNetworkUse(t *testing.T) {
	...
	r := &runner{ctx: context.Background(), out: &output, errOut: &output, store: store}
	client := beeapi.New("https://beeapi.dev")
	account, err := r.saveOAuthSession(beeapi.OAuthMetadata{Issuer: beeapi.OAuthIssuerDev}, beeapi.OAuthToken{
...
func TestOAuthSessionRejectsLegacyMetadataThatLacksIssuerBoundSecrets(t *testing.T) {
	t.Setenv("GETBEE_DISABLE_KEYRING", "1")
	store := &state.Store{Dir: t.TempDir()}
	r := &runner{ctx: context.Background(), out: io.Discard, errOut: io.Discard, store: store}
	client := beeapi.New(beeapi.OAuthIssuerDev)
	account, err := r.saveOAuthSession(beeapi.OAuthMetadata{Issuer: beeapi.OAuthIssuerDev}, beeapi.OAuthToken{
		AccessToken: "boa_current", RefreshToken: "bor_current", TokenType: "DPoP", ExpiresIn: 600,
		Scope: "account:profile:read offline_access",
	}, client)
	if err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(store.OAuthAccountPath(), append(b, '\n'), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, _, err := r.loadOAuthSession(); err == nil || !strings.Contains(err.Error(), "重新连接") {
		t.Fatalf("legacy OAuth metadata was reused: %v", err)
	}
}
