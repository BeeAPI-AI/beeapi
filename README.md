...
beeapi run claude-desktop      打开已配置的 Claude Desktop
beeapi token print --agent codex 仅向 Codex 的 BeeAPI provider 提供已分配的 Key
```

配置完成后可直接使用工具原本的命令，不需要 `beeapi run`、工具自身的额外 profile 或 Shell 函数。GetBeeAPI 的每个“配置方案”只属于一个工具，并记录该工具的入口、Key、模型与专属参数；切换时先完整备份原文件，再只更新对应工具的 BeeAPI 连接字段，其他工具以及权限、MCP、主题、插件等无关设置保持不变。`beeapi run <工具>` 仍保留为兼容和排障入口。升级自 v0.2.x 时，旧的多工具方案仍可读取，并会按工具独立投影和切换。
