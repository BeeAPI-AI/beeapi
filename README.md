...
2. 通过 BeeAPI OAuth 连接账户。桌面端使用 Authorization Code + PKCE 与本机回调，SSH/无桌面环境使用标准 Device Grant 并始终显示完整授权网址。CLI 先读取账户余额、API Key 元数据与各 Key 的模型能力；用户在终端明确选中 Key 后才执行一次性导出。也可直接粘贴单个 API Key 作为兼容回退。
...
- OAuth 连接可直接读取账户级余额；旧连接则回退 API Key 鉴权的只读 `/v1/usage`。两种方式都不会发送模型请求或产生用量；首页使用短期缓存，详情页可主动刷新并检查每个 Key 的可用状态。
